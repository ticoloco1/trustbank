...
export default function CVProTemplate({
  ...
  return (
    <div
      ...
      style={{ ["--cv-primary" as string]: primary }}
    >
      {slug && (
      ...
      <div className="tb-cv-wrapper">
        <header className="tb-cv-header">
          ...
          <div className="tb-cv-header-content">
            {site.feed_image_1 && <SafeImage src={site.feed_image_1} alt={name} className="tb-cv-avatar" />}
            <h1 className="tb-cv-name">{name}</h1>
            {headline && <p className="tb-cv-headline">{headline}</p>}
          </div>
        </header>
        {sections.map((page) => (
